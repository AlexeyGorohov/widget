(function () {
  ...
  var CBWidgetPopup = function () {
    ...
    var seatingArr = _toConsumableArray$1(Array(100).keys()).map(function (item) {
      return {
        id: item,
        price: 120,
        disabled: false
    ...
    var file = "src/view/seating/Index.svelte";
    ...
    function create_each_block_1(ctx) {
      ...
      dispatch_dev("SvelteRegisterBlock", {
        ...
        type: "each",
        source: "(19:2) {#each selectedSeat as item}",
        ctx: ctx
      });
      return block;
    ...
    function create_each_block(key_1, ctx) {
      ...
      dispatch_dev("SvelteRegisterBlock", {
        ...
        type: "each",
        source: "(23:4) {#each $seating as seat (seat.id)}",
        ctx: ctx
      });
      return block;
    }
    ...
    function create_fragment(ctx) {
      ...
      var each_value_1 = ctx.selectedSeat;
      var each_blocks_1 = [];
      ...
      for (var _i2 = 0; _i2 < each_value.length; _i2 += 1) {
        var child_ctx = get_each_context(ctx, each_value, _i2);
        var key = get_key(child_ctx);
        each1_lookup.set(key, each_blocks[_i2] = create_each_block(key, child_ctx));
      }
        ...
        p: function update(changed, ctx) {
          ...
          if (changed.selectedSeat) {
            ...
            for (_i7 = 0; _i7 < each_value_1.length; _i7 += 1) {
              var _child_ctx = get_each_context_1(ctx, each_value_1, _i7);

              if (each_blocks_1[_i7]) {
                each_blocks_1[_i7].p(changed, _child_ctx);
              } else {
                each_blocks_1[_i7] = create_each_block_1(_child_ctx);

                each_blocks_1[_i7].c();

                each_blocks_1[_i7].m(div1, null);
              }
            }
    ...
    function instance($$self, $$props, $$invalidate) {
      ...
      component_subscribe($$self, seating, function ($$value) {
        $seating = $$value;
        $$invalidate('$seating', $seating);
      });

      var click_handler = function click_handler(_ref, e) {
        var seat = _ref.seat;
        return addToOrder(seat);
      };
      ...
      $$self.$$.update = function () {
        ...
        if ($$dirty.$seating) {
          $$invalidate('selectedSeat', selectedSeat = $seating.filter(function (item) {
            return item.disabled === true;
          }));
        }

        if ($$dirty.selectedSeat) {
          $$invalidate('sum', sum = selectedSeat.reduce(function (sum, item) {
    ...
    function (_SvelteComponentDev2) {
      _inherits(App, _SvelteComponentDev2);
      ...
      function App(options) {
        ...
        _this2 = _possibleConstructorReturn(this, _getPrototypeOf(App).call(this, options));
        init(_assertThisInitialized(_this2), options, instance$1, create_fragment$1, safe_not_equal, ["CBID"]);
        dispatch_dev("SvelteRegisterComponent", {
  ...
  /*#__PURE__*/
  function () {
    function CBWidget(CBID) {
      _classCallCheck(this, CBWidget);
    ...
    _createClass(CBWidget, [{
      key: "getModalWidget",
      value: function getModalWidget() {
        return document.querySelector('.widget-cb');
      }
    }, {
      key: "setEventsBtns",
      value: function setEventsBtns() {
        var _this = this;
        ...
        btns.forEach(function (btn) {
          btn.addEventListener('click', function (e) {
            ...
            _this.getModalWidget().classList.add('show');
          });
        });
      }
    }, {
      key: "observeModal",
      value: function observeModal() {
        var _this2 = this;

        var widget = this.getModalWidget();
        var options = {
          attributes: true,
          attributeOldValue: true
        };

        var callbackMutation = function callbackMutation(mutations) {
          mutations.forEach(function (_ref) {
            var target = _ref.target;

            if (target.classList.contains('show')) {
              _this2.setEventOutsideWidget();
            } else {
              _this2.removeEventOutsideWidget();
            }
          });
        };
      ...
      key: "setEventOutsideWidget",
      value: function setEventOutsideWidget() {
        document.addEventListener('click', this.listenerWidget.bind(this));
      }
    }, {
      key: "listenerWidget",
      value: function listenerWidget(event) {
        var widget = this.getModalWidget();
        var isClickInside = widget.contains(event.target);
        console.log(isClickInside, 'isClickInside'); // if (!isClickInside) {
